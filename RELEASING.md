...
Canonical validation is `python3.11 -B scripts/test_fork.py -v` plus the
unchanged token-report tests. Build with `scripts/package_smart.py`, not the
legacy six-role packaging entry point. Version and user marker must match.
